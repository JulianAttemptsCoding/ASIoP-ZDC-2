...
This blocker was superseded on 2026-07-11 after the user directed use of the prior
`ML ZDC all` implementation and its `hit_unit_evidence.json`. The accepted contract is documented in
`outputs/reports/hit_unit_resolution.md`. This file is preserved as audit history.
...
- Local ROOT file: `C:\Users\Julia\OneDrive\Desktop\coding\ASIoP\ML ZDC all 1\myTree_20251117_765k_0to300GeV_neutron_All.root`
- Local file size: 25,022,001,408 bytes; SHA-256
  `b7c666040e42352e158a9a3f78158d147cb2e056c6c88248d892c956f5c7b533`; CRC32C
  `lCVUvQ==`; MD5 `sQdukAxxUbb+Fh3JDOUjLQ==`.
- Vertex/GCS staged object:
  `gs://asiop-zdc-1-zdc-reco-us-central1/data/myTree_20251117_765k_0to300GeV_neutron_All.root`
  in the `US-CENTRAL1` bucket. The local CRC32C and size match the staged object, so no duplicate
  23.3 GiB upload was performed.
...
- Local-path metadata CLI: `python -m zdc_hybrid.cli preflight --config configs/study.yaml --data
  C:\Users\Julia\OneDrive\Desktop\coding\ASIoP\ML ZDC all 1\myTree_20251117_765k_0to300GeV_neutron_All.root
  --output-dir outputs\local_input_preflight` returned nonzero intentionally and wrote a matching
  local-path blocker.
- Vertex data staging manifest: `outputs/reports/vertex_data_staging.json`.
- ROOT metadata: `outputs/preflight/root_metadata.json`.
- Local-path ROOT metadata: `outputs/local_input_preflight/preflight/root_metadata.json`.
...
The staged ROOT object can later be deleted with:
